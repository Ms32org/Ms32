...
def download(name:str,url:str) -> bool:
    try:
        if not which("wget"): 
            sbrun(["winget","install","wget","--accept-package-agreements","--silent"],shell=True)#download wget
            log("Downloaded Wget")
...
def unmuter() -> None:
    try:
        ...
        log(f"Unmuter thread error occured:\t{e}",state="WARN")
